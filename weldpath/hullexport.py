    ...
    for name in list(env.getLinkNames()):
        ...
        for col in link.collision:
            ...
            for part in _parts(col.geometry):
                ...
                if not tris:
                    continue
                V = (V @ tf[:3, :3].T + tf[:3, 3]) * unit
                hulls.append((V, tris))
        if not hulls:
            continue

        path = os.path.join(out_dir, f"{name}.obj").replace("\\", "/")
        faces = sum(len(t) for _, t in hulls)
        with open(path, "w", encoding="utf-8") as fh:
            fh.write(f"# weldpath collision geometry for link {name}\n")
            fh.write(f"# {len(hulls)} convex pieces, {faces} triangles, "
                     f"world coordinates at the current state, units={man.units}\n")
            offset = 1
            for hi, (V, tris) in enumerate(hulls):
                fh.write("o hull_%05d\n" % hi)
                for x, y, z in V:
                    fh.write("v %.6f %.6f %.6f\n" % (x, y, z))
                for a, b, c in tris:
